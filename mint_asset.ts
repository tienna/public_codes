...
const asset_name = "C2VN"
const policyId = lucid.utils.mintingPolicyToId(mintingPolicy);
const unit = policyId + fromText(asset_name);
const filename = "ipfs://QmdpcDnQj5u54JZ5ZxQMLXjajZAeAXRqHs7dNGvh7wVhq1";


const metadata = {
    [policyId]: {
        [asset_name]: {
            "description": "C2VN: Token- we break the blocks",
            "id": "1",
            "image": filename,
            "name": asset_name
        }
...
const tx = await lucid.newTx()
    .mintAssets({ [unit]: BigInt(500) }) //tên và số lượng
